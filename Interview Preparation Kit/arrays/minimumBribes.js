...
function minimumBribes(q) {
    ...
    for (let index = 0; index < q.length; index++) {
        const currentValue = q[index];
        const normalizedIndex = index + 1;

        if (currentValue > normalizedIndex && (currentValue - normalizedIndex) > 2) {
    ...
    for (let index = 0; index < q.length; index++) {
        ...
        // Check if value does not belong. E.g. normalizedIndex != currentValue
        if (normalizedIndex !== currentValue) {

            // Find element that does belong from inside the array.
            let tempIndex = index + 1;
            let tempValue = q[tempIndex];

            while (normalizedIndex !== tempValue) {
                tempIndex++;
                tempValue = q[tempIndex]; 
            }

            // Once found. Swap back elements in place.
            while (tempIndex !== index) {
                if (q[tempIndex - 1] > tempValue) {
                    minimumBribes++;
                }

                // Perform Swap.
                let tempTempValue = tempValue;
                q[tempIndex] = q[tempIndex - 1];
                q[tempIndex - 1] = tempTempValue;
                tempIndex--;
            }
        }        
    }
